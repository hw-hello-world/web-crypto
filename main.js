...
const encrypt = () => {
  ...
  importMasterKey(masterPassword).then(function (derivedMasterKey) {
    ...
    Promise.all([
      ...
      window.crypto.subtle.generateKey(rsaAlg, true, ["encrypt", "decrypt"]),
      ...
      .then(function (result) {
        ...
        return crypto.subtle
          .exportKey("jwk", rsaKeys.publicKey)
          ...
          .then(function () {
            ...
            return crypto.subtle
              .exportKey("jwk", rsaKeys.privateKey)
          ...
          .then(function () {
            ...
            return window.crypto.subtle
              .encrypt({ name: "AES-GCM", iv }, kek, enc.encode(privateKeyJson))
              .then(function (encryptedPrivateKey) {
                dbSession.encryptedPrivateKeyIV = iv;
          ...
          .then(function (vaultKeyJwk) {
            ...
            return window.crypto.subtle
              .importKey("jwk", dbSession.publicKeyJwk, rsaAlg, "false", [
                "encrypt",
              ])
          ...
          .then(function () {
            ...
            sessionStorage.setItem(
              "my_pub_key",
              JSON.stringify(dbSession.publicKeyJwk)
            );
            sessionStorage.setItem(
              "my_private_encrypted_key",
              bufferToBase64(dbSession.encryptedPrivateKeyIV) +
                bufferToBase64(dbSession.encryptedPrivateKey)
            );
            sessionStorage.setItem(
              "my_vault_encrypted_key",
              bufferToBase64(dbSession.encryptedVaultKey)
            );
          });
      })
      .then(function () {
        ...
        if (publicKeyJwk) {
          ...
          window.crypto.subtle
            .importKey("jwk", publicKeyJwk, rsaAlg, "false", ["encrypt"])
            .then(function (publicKey) {
              ...
              return window.crypto.subtle
                ...
                .then(function (ciphertext) {
                  ...
                  console.log(
                    ...
                    cipherBuffer.byteLength
                  );
                  console.log("encrypted string:", bufferToStr(cipherBuffer));
                  console.log(
                    "encrypted string size:",
                    bufferToStr(cipherBuffer).length
                  );
                  console.log("encrypted base64 string:", cipherStrBase64);
                  console.log(
                    "encrypted base64 string size:",
                    cipherStrBase64.length
                  );
                  console.groupEnd();

                  const finalResult = bufferToBase64(iv) + cipherStrBase64;
                  document.getElementById(
                    "app_encrypted_password"
...
const decrypt = () => {
  ...
  const cipherStr = document.getElementById("app_encrypted_password")
    .textContent;
  const masterPassword = document.getElementById("master_key").value;
  const encryptedPrivateKey = sessionStorage.getItem(
    "my_private_encrypted_key"
  );

  importMasterKey(masterPassword).then(function (derivedMasterKey) {
    return createKeyEncryptionKey(derivedMasterKey, cache.kekSalt, [
      "encrypt",
      "decrypt",
    ])
      .then(function (kek) {
        return window.crypto.subtle.decrypt(
          {
            name: "AES-GCM",
            iv: base64ToBuffer(encryptedPrivateKey.slice(0, 16)),
          },
          kek,
          base64ToBuffer(encryptedPrivateKey.slice(16))
        );
      })
      ...
      .then(function (privateKey) {
        const encryptedVaultKey = sessionStorage.getItem(
          "my_vault_encrypted_key"
        );
        return window.crypto.subtle.decrypt(
          rsaAlg,
      ...
      .then(function (vaultKey) {
        const ivBuffer = base64ToBuffer(cipherStr.slice(0, 16));
        const bufferForDecrypt = base64ToBuffer(cipherStr.slice(16));
      ...
      .then(function (decryptedBuffer) {
        ...
        console.groupEnd();
        document.getElementById(
          "app_decrypted_password"
        ).textContent = decryptedStr;
        const originalPassword = document.getElementById("app_password").value;
        document.getElementById("assertion").textContent =
          decryptedStr === originalPassword;
      });
  });
};
